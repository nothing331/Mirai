...
**Outcome.** A user can choose a deterministic or generative edit, outline its focus region, receive conservative closed-contour cleanup, refine the mask with Add area or Subtract area, and clear or redraw it inside one purpose-led Lasso inspector. Lasso Edit appears in the high-visibility AI group at the start of the rail because Remove, Replace, and Style are AI workflows; its local Color operation remains available alongside them.

**Working flow.** The Lasso inspector leads with the edit operation and its color or instruction, then presents selection as the area where that operation applies. Before a mask exists, it asks the user to draw a closed shape on the canvas. Once selected, Add area and Subtract area expose brush size and edge softness; Redraw selection starts the contour over and Clear selection removes it. Clearing from either the inspector or compact canvas chip returns interaction to closed-shape drawing. Source-space pointer samples form selection geometry. Cleanup removes unreliable contour artifacts and reports diagnostics. Closed-shape drawing creates a region; Add and Subtract rasterize source-space refinement strokes into the same full-resolution mask.
