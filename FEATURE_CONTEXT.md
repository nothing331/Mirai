...
**Outcome.** A user can adapt the current image to YouTube, Instagram, Story/Reel, square, portrait, or landscape formats without drawing a selection or entering dimensions. Extend is a first-class rail tool with the `X` shortcut. Smart Reframe is the default and may conservatively trim low-value outer space; Keep full image forbids cropping. The proposed crop, source placement, output dimensions, and generated area are shown before the image request.

**Working flow.** The inspector captures a versioned target preset, Smart/Keep-full strategy, and optional direction. A separate planning request analyzes the immutable source once, returning normalized subject, text, horizon, visual-center, negative-space, and edge-continuation evidence. Preset changes reuse that analysis. Application-owned deterministic geometry turns the evidence into integer source-crop, unscaled source-placement, target-canvas, expansion-inset, and seam geometry; low-confidence analysis falls back to preserving the complete image. Generation validates the frozen plan, constructs an aligned transparent provider canvas and same-size alpha mask, and calls GPT Image 2 at fixed low quality. The complete provider proposal owns new pixels and the narrow seam, while server compositing restores the retained source core exactly. The output-sized effective mask, final dimensions, plan, analysis, instruction, and provider correlation enter shared preview and acceptance. Accept creates one dimension-changing version and one operation; undo/redo, persistence, and export follow each version's own dimensions.

**Ownership and rules.** Versioned presets and planning contracts live in `src/shared/`; the semantic planner identifies evidence but cannot choose final coordinates. The deterministic solver owns crop and placement, never scales retained source content, and refuses crops that cannot contain must-preserve subjects or important text. Server routes own plan validation, provider-only scaling/alignment, prompt construction, mask conversion, candidate normalization, and exact-core restoration. UI components request planning/generation but never call OpenAI or write history. Extend does not consume the current selection, does not overwrite the original, and cannot accept a stale plan after the current version changes. Pending paint must be resolved first.

**Failures and recovery.** Planner, schema, plan-validation, provider, dimension, or compositing failures do not advance history. Planner confidence below the policy threshold produces a safe preserve-all plan. A failed semantic planning request can be retried or replaced with Keep full image after planning becomes available. Generation produces one reviewable candidate; Adjust returns to the retained plan, Discard removes only the proposal, and late results for a superseded input version are ignored. The real-provider session allowance counts generation but not frame planning.

**Dependencies and limits.** OpenAI planning defaults to `gpt-5.6-luna`; image generation is `gpt-image-2` with low quality fixed for Extend. Fake adapters provide deterministic local development. v1 has six maintained aspect presets, one candidate, automatic placement, and an optional text direction. Freeform handles, custom dimensions, manual source dragging, multiple variants, batch generation, reference images, automatic semantic post-validation, and exact platform export resolutions are deferred. The seam is intentionally editable; exact preservation applies to the retained core outside it.
