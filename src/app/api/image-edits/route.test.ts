...
describe("image edit route orchestration", () => {
  ...
  it("plans Replace before starting the image editor", async () => {
    ...
    mocks.edit.mockResolvedValue({ candidatePng: new Uint8Array([1, 2, 3]), providerRequestId: "image-1" });
    ...
    expect(mocks.edit).toHaveBeenCalledWith(expect.objectContaining({ plan }), undefined);
    expect(mocks.plan.mock.invocationCallOrder[0]).toBeLessThan(mocks.edit.mock.invocationCallOrder[0]);
    expect(payload).toMatchObject({ providerRequestId: "image-1", imageGenerationAttempted: true });
  });
  ...
  it("does not plan Restyle requests", async () => {
    mocks.edit.mockResolvedValue({ candidatePng: new Uint8Array([1]), providerRequestId: "image-2" });
