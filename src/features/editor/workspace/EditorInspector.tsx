...
import { Activity, Brush, ChevronDown, Eraser, Palette, Shield, Sparkles, WandSparkles } from "lucide-react";
    ...
    <div className="inspector-enter flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
        ...
        {(state.tool === "brush" || state.tool === "eraser") && (
          <section className="grid gap-3 border-t border-line py-3" aria-label={`${state.tool} settings`}>
            <div className="flex items-center gap-2 text-[11px] font-bold"><span className="grid size-6 place-items-center bg-[#e8e5dc]">{state.tool === "brush" ? <Brush className="size-3.5" /> : <Eraser className="size-3.5" />}</span>{state.tool === "brush" ? "Brush" : "Eraser"}</div>
            <CompactSlider label="Brush size" value={`${state.brushSize}px`} min={4} max={160} step={1} sliderValue={state.brushSize} onChange={state.setBrushSize} />
        ...
        {state.selectionDiagnostics?.warnings.length ? (
          <details className="border-t border-line py-3">
            ...
            <p className="mt-2 text-[10px] leading-relaxed text-muted">The original contour was preserved where automatic cleanup was uncertain. Refine it with Brush or Eraser if needed.</p>
          </details>
        ) : null}
      ...
      <div className="border-t border-line p-3">
        {state.editType === "recolor" ? (
          <button type="button" data-testid="apply-edit" className="flex h-10 w-full items-center justify-center gap-2 bg-acid text-xs font-bold text-ink outline-none hover:bg-ink hover:text-acid focus-visible:ring-2 focus-visible:ring-accent" onClick={onGenerate}><Sparkles className="size-4" />Preview color</button>
        ) : (
          <button type="button" data-testid="generate-edit" className="flex h-10 w-full items-center justify-center gap-2 bg-acid text-xs font-bold text-ink outline-none hover:bg-ink hover:text-acid focus-visible:ring-2 focus-visible:ring-accent disabled:pointer-events-none disabled:opacity-45" disabled={processing || requestLimitReached} onClick={onGenerate}>
