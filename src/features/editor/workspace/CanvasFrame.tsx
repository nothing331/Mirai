...
import { Check, Focus, ImagePlus, LoaderCircle, Sparkles, X } from "lucide-react";
...
export function CanvasFrame({ busyAction, onUpload, onGenerateAsset }: { busyAction: BusyAction; onUpload: (event: ChangeEvent<HTMLInputElement>) => void; onGenerateAsset: () => void }) {
            ...
            <div className="grid w-full max-w-xl gap-6 text-center">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <button data-testid="open-asset-generator" type="button" className="group grid min-h-36 place-items-center border border-acid bg-acid p-5 text-ink hover:bg-paper" onClick={onGenerateAsset}>
                  <span><Sparkles className="mx-auto mb-3 size-5 transition-transform group-hover:scale-125" /><strong className="block text-sm">Create with AI</strong><small className="mt-1 block font-mono text-[8px] uppercase tracking-wider text-muted">Mark · image · transform</small></span>
                </button>
              </div>
            </div>
          </div>
