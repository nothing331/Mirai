...
export async function saveEditorProject(state: { projectId: string | null; projectName: string; originalVersionId: string | null; currentVersionId: string | null; versions: ImageVersion[]; operations: EditOperation[]; maskAssets: MaskAsset[]; overlayAssets: OverlayImageAsset[] }) {
...
export async function openSavedProject(id: string) {
  ...
  const project = await response.json() as { id: string; name: string; originalVersionId: string; currentVersionId: string; versions: Array<Omit<ImageVersion, "pixels">>; operations: EditOperation[]; maskAssets: Array<{ id: string; width: number; height: number; alpha: number[] }>; overlayAssets?: Array<Omit<OverlayImageAsset, "pixels">> };
